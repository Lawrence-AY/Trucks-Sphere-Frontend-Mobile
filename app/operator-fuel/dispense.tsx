...
export default function FuelDispenseScreen() {
  ...
  // Only show jobs that are NOT completed — completed jobs are fully removed from this list
  const activeDeliveries = useMemo(() => {
    return deliveries.filter(
      (d) =>
        !["completed", "delivered", "received", "cancelled"].includes(
          d.status,
        ) && !store.isJobCompleted(d.jobId || d.id),
    );
  }, [deliveries, store.completedJobIds]);
  ...
  const handleRequestAuthorization = async () => {
    if (!store.activeJob) {
      showToast(
        "error",
        "No Job Selected",
        "Please select an active job first.",
      );
      return;
    }
  ...
  const handleFlowSubmit = async () => {
    ...
    if (!store.activeJob) {
      showToast("error", "No Job", "No active job selected.");
      return;
    }
    ...
    try {
      ...
      showToast(
        "success",
        "Fuel Dispensed",
        `${amount.toFixed(1)} litres recorded as ${fuelId}. Auth PIN: ${authCodeRef}. Job card removed from active jobs.`,
      );
        ...
        {loading ? (
          ...
          <DataCard>
            ...
            >
              ...
              >
                {activeDeliveries.length} active job
                {activeDeliveries.length !== 1 ? "s" : ""} available
              </Text>
            </View>
            <Text
          ...
          <EmptyState
            ...
            subtitle="No active delivery orders available for fuel dispensing."
          />
        )}
      </PageShell>
                ...
                <>
                  {activeDeliveries.length ? (
                      ...
                      return (
                        ...
                        >
                          ...
                          <View style={{ flex: 1 }}>
                            ...
                            >
                              Vendor: {job.vendorName || "N/A"} · Material:{" "}
                              {job.materialName || "N/A"}
                            </Text>
                            {existingFuel > 0 && (
                              <View
                                style={[
                    ...
                    <EmptyState
                      ...
                      title="No active jobs"
                      subtitle="No active delivery orders available."
                    />
                  )}
                </>
              )}
