...
const CATEGORIES = [
  ...
  { key: 'quarry-ops', label: 'Quarry Ops', icon: 'hammer-outline', color: '#D97706' },
  { key: 'site-ops', label: 'Site Ops', icon: 'business-outline', color: '#059669' },
];
...
export default function ReportsScreen() {
  ...
  const handleDownloadCSV = async () => { setCsvDownloading(true); try { await downloadCategoryCSV(activeTab, { filter: filter !== 'all' ? filter : undefined }); } catch {} finally { setCsvDownloading(false); } };
  ...
  return (
    <PageShell>
      ...
      {loading ? (<DataCard><View style={{ flexDirection: 'row', alignItems: 'center', gap: Spacing.md }}><ActivityIndicator color={colors.primary} /><Text style={{ fontSize: 14, color: colors.textMuted }}>Loading live data...</Text></View></DataCard>) : d ? (<><View style={styles.metricsGrid}>{renderCategoryCards(activeTab, d, colors, metrics)}</View>{d.preview && d.preview.length > 0 && (<><SectionTitle title="Recent Records" /><DataCard>{renderPreviewTable(activeTab, d.preview, colors, metrics)}</DataCard></>)}</>) : (<DataCard><Text style={{ fontSize: 14, color: colors.textMuted }}>No data available.</Text></DataCard>)}
...
function renderCategoryCards(tab: string, d: any, colors: any, m: any) {
  switch (tab) {
    ...
    case 'purchase-orders': return (<><MetricCard icon="document-text-outline" label="Total POs" value={d.total ?? 0} color="#0EA5E9" /><MetricCard icon="time-outline" label="Open" value={d.open ?? 0} color="#F59E0B" /><MetricCard icon="checkmark-circle-outline" label="Fulfilled" value={d.fulfilled ?? 0} color="#10B981" /></>);
    case 'quarry-ops': return (<><MetricCard icon="hammer-outline" label="Quarry Visits" value={d.total ?? 0} color="#D97706" /><MetricCard icon="time-outline" label="Active (In)" value={d.active ?? 0} color="#F59E0B" /><MetricCard icon="checkmark-circle-outline" label="Dispatched" value={d.completed ?? 0} color="#10B981" /><MetricCard icon="scale-outline" label="Gross W.In" value={`${(d.totalTonnage ?? 0).toFixed(1)}T`} color="#D97706" /></>);
    case 'site-ops': return (<><MetricCard icon="business-outline" label="Site Arrivals" value={d.total ?? 0} color="#059669" /><MetricCard icon="time-outline" label="Active (In)" value={d.active ?? 0} color="#F59E0B" /><MetricCard icon="checkmark-circle-outline" label="Completed" value={d.completed ?? 0} color="#10B981" /><MetricCard icon="scale-outline" label="Net Weight" value={`${(d.totalNet ?? 0).toFixed(1)}T`} color="#059669" /></>);
    default: return null;
  }
}
...
function renderPreviewTable(tab: string, items: any[], colors: any, m: any) {
  ...
  switch (tab) {
    case 'deliveries': return items.map((d, i) => (<View key={i} style={{ marginBottom: i < items.length - 1 ? Spacing.sm : 0, paddingBottom: i < items.length - 1 ? Spacing.sm : 0, borderBottomWidth: i < items.length - 1 ? 1 : 0, borderBottomColor: colors.border }}><Text style={{ fontSize: 13, fontWeight: '700', color: colors.text }}>{d.jobId || d.id}</Text><Text style={{ fontSize: 12, color: colors.textMuted }}>{d.vendorName || '—'} · {d.plateNumber || '—'} · {d.materialName || '—'}</Text><View style={{ flexDirection: 'row', gap: Spacing.sm, marginTop: 2 }}><Text style={{ fontSize: 11, color: '#2563EB', fontWeight: '700' }}>{(d.netWeight || d.quantityDelivered) ? `${Number(d.netWeight || d.quantityDelivered).toFixed(1)}T` : '—'}</Text><Text style={{ fontSize: 11, color: colors.textTertiary }}>{d.status || '—'}</Text></View></View>));
    ...
    case 'purchase-orders': return items.map((p, i) => (<View key={i} style={{ marginBottom: i < items.length - 1 ? Spacing.sm : 0, paddingBottom: i < items.length - 1 ? Spacing.sm : 0, borderBottomWidth: i < items.length - 1 ? 1 : 0, borderBottomColor: colors.border }}><Text style={{ fontSize: 13, fontWeight: '700', color: colors.text }}>{p.poNumber || p.id}</Text><Text style={{ fontSize: 12, color: colors.textMuted }}>{p.vendorName || '—'} · {p.materialName || '—'} · {p.quantity || 0}T</Text><Text style={{ fontSize: 11, color: colors.textTertiary }}>{p.status || '—'}</Text></View>));
    case 'quarry-ops': return items.map((d, i) => (<View key={i} style={{ marginBottom: i < items.length - 1 ? Spacing.sm : 0, paddingBottom: i < items.length - 1 ? Spacing.sm : 0, borderBottomWidth: i < items.length - 1 ? 1 : 0, borderBottomColor: colors.border }}><Text style={{ fontSize: 13, fontWeight: '700', color: colors.text }}>{d.jobId || d.id}</Text><Text style={{ fontSize: 12, color: colors.textMuted }}>{d.driverName || '—'} · {d.plateNumber || '—'} · {d.materialName || '—'}</Text><View style={{ flexDirection: 'row', gap: Spacing.sm, marginTop: 2 }}><Text style={{ fontSize: 11, color: '#D97706', fontWeight: '700' }}>In: {d.weighInWeight ? `${Number(d.weighInWeight).toFixed(1)}T` : '—'}</Text><Text style={{ fontSize: 11, color: '#059669', fontWeight: '700' }}>Out: {d.weighOutWeight ? `${Number(d.weighOutWeight).toFixed(1)}T` : '—'}</Text><Text style={{ fontSize: 11, color: '#2563EB', fontWeight: '700' }}>Net: {d.netWeight ? `${Number(d.netWeight).toFixed(1)}T` : '—'}</Text></View></View>));
    case 'site-ops': return items.map((d, i) => (<View key={i} style={{ marginBottom: i < items.length - 1 ? Spacing.sm : 0, paddingBottom: i < items.length - 1 ? Spacing.sm : 0, borderBottomWidth: i < items.length - 1 ? 1 : 0, borderBottomColor: colors.border }}><Text style={{ fontSize: 13, fontWeight: '700', color: colors.text }}>{d.jobId || d.id}</Text><Text style={{ fontSize: 12, color: colors.textMuted }}>{d.driverName || '—'} · {d.plateNumber || '—'} · {d.materialName || '—'}</Text><View style={{ flexDirection: 'row', gap: Spacing.sm, marginTop: 2 }}><Text style={{ fontSize: 11, color: '#D97706', fontWeight: '700' }}>In: {d.siteWeighInWeight ? `${Number(d.siteWeighInWeight).toFixed(1)}T` : '—'}</Text><Text style={{ fontSize: 11, color: '#059669', fontWeight: '700' }}>Out: {d.siteWeighOutWeight ? `${Number(d.siteWeighOutWeight).toFixed(1)}T` : '—'}</Text><Text style={{ fontSize: 11, color: '#2563EB', fontWeight: '700' }}>Net: {d.siteNetWeight || d.netWeight ? `${Number(d.siteNetWeight || d.netWeight).toFixed(1)}T` : '—'}</Text></View></View>));
    default: return null;
  }
}
